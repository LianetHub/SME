  ...
  foreach ($sidebars_widgets as $sidebar => $widgets) {
    if (is_array($widgets)) {
      foreach ($widgets as $key => $widget_id) {
        if (strpos($widget_id, 'tag_cloud') !== false) {
...
function cme_set_post_views($postID)
{
...
function cme_get_post_views($postID)
{
  $count = get_post_meta($postID, 'post_views_count', true);
  return $count ? (int) $count : 0;
}

function cme_increment_views_ajax()
{
  if (isset($_POST['post_id']) && is_numeric($_POST['post_id'])) {
    cme_set_post_views((int)$_POST['post_id']);
